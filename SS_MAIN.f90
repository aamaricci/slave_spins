...
contains
  ...
  subroutine ss_init_hk(hk_user,wtk_user,hloc)
    ...
    isetup=.false.
    !
    Htmp    = sum(Hk_user,dim=3)
    where(abs(Htmp)<1d-6)Htmp=zero
    if(present(Hloc).AND.(sum(abs(Htmp))>1d-12))stop "ss_init_hk error: Hloc seems to be present twice: in _Hloc and _Hk"
    !
    !> Set ss_Hk
    do ik=1,Nk
       select case(Nspin)
    ...
    case (1)
       ...
       if(present(Hloc))ss_Hloc = kron(pauli_0,Hloc)
    case (2)
       ss_Hloc = Htmp
       if(present(Hloc))ss_Hloc = Hloc           !local part
    end select
    !
    call ss_get_lambda0()
    !
  ...
  subroutine ss_solve()
    ...
    !
    lambda = ss_lambda(:Nspin*Norb)
    !
    select case(solve_method)
    case ("broyden")       
       call broyden1(ss_solve_function,lambda,tolf=constraint_tolerance)
    case ("hybrd")
       call fsolve(ss_solve_function,lambda,tol=constraint_tolerance)
    case default
  ...
  contains
    ...
    function ss_solve_function(lambda) result(fss)
      ...
      logical                         :: z_converged
      real(8)                         :: zeta(Ns),Fzeta(Ns),ss_zeta_prev(Ns),z_error
      !
      select case(Nspin)
      case (1)
